    ...
    if(lastBidder){
        ...
        if(lastBidder.discord_id != user.discord_id) {
            await ctx.direct(lastBidder, `Another player has outbid you on card ${formatName(ctx.cards[auc.card])}
                To remain in the auction, try bidding higher than ${auc.price} ${ctx.symbols.avocados}
        ...
        await ctx.direct(author, `a player has bid on your auction \`${auc.id}\` for card 
            ${formatName(ctx.cards[auc.card])} with minimum ${auc.price} ${ctx.symbols.avocados}!`, 'green')
    }

    return ctx.reply(user, `you successfully bid on auction \`${auc.id}\` with **${bid}** ${ctx.symbols.avocados}!`)
}
...
const finish_aucs = async (ctx, now) => {
    ...
    if(lastBidder) {
        ...
        await ctx.direct(author, `you sold ${formatName(ctx.cards[auc.card])} on auction \`${auc.id}\` for **${auc.price}** ${ctx.symbols.avocados}`)
        return ctx.direct(lastBidder, `you won auction \`${auc.id}\` for card ${formatName(ctx.cards[auc.card])}!
            You ended up paying **${Math.round(auc.price)}** ${ctx.symbols.avocados} and got **${Math.round(auc.highbid - auc.price)}** ${ctx.symbols.avocados} back.
            ${tback > 0? `You got additional **${tback}** ${ctx.symbols.avocados} from your equipped effect` : ''}`)
    } else {
        if(!findSell){
            const sellDB = await new AuditAucSell()
            sellDB.user = author.discord_id
...
const paginate_auclist = (ctx, user, list) => {
    ...
    list.map((auc, i) => {
        ...
        pages[Math.floor(i/10)] += `${char} [${diffstr}] \`${auc.id}\` [${auc.price}${ctx.symbols.avocados}] ${formatName(ctx.cards[auc.card])}\n`
    })
