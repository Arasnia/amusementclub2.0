...
cmd(['guild', 'status'], (ctx, user) => {
    ...
    resp.push(`Building maintenance: **${cost}** ${ctx.symbols.avoamusement}/day`)

    if(ctx.guild.discount > 0) {
        resp.push(`Maintenance discount: **${ctx.guild.discount * 100}%**`)
        resp.push(`Subtotal after discounts: **${total}** ${ctx.symbols.avoamusement}/day`)
    }
    resp.push(`Current finances: **${ctx.guild.balance}** ${ctx.symbols.avoamusement}`)
    ...
    return ctx.send(ctx.msg.channel.id, {
        ...
        fields: [{name: `Maintenance breakdown`, value: ctx.guild.buildings.map(x => {
            ...
            return `[\`${heart}\` ${x.health}] **${item.name}** level **${x.level}** costs **${item.levels[x.level - 1].maintenance}** ${ctx.symbols.avoamusement}/day`
        }).join('\n')}],
...
cmd(['guild', 'upgrade'], async (ctx, user, arg1) => {
    ...
    if(user.exp < level.price)
        return ctx.reply(user, `you have to have at least **${level.price}** ${ctx.symbols.avoamusement} to upgrade this building`, 'red')

    const question = `Do you want to upgrade **${item.name}** to level **${building.level + 1}** for **${level.price}** ${ctx.symbols.avoamusement}?`
...
cmd(['guild', 'donate'], async (ctx, user, arg1) => {
    ...
    if(user.exp < amount)
        return ctx.reply(user, `you don't have **${amount}** ${ctx.symbols.avoamusement} to donate`, 'red')

    const question = `Do you want to donate **${amount}** ${ctx.symbols.avoamusement} to **${ctx.discord_guild.name}**?`
    return ctx.pgn.addConfirmation(user.discord_id, ctx.msg.channel.id, {
        question,
        force: ctx.globals.force,
        onConfirm: async (x) => {
            ...
            return ctx.reply(user, `you donated **${amount}** ${ctx.symbols.avoamusement} to **${ctx.discord_guild.name}**!
                This now has **${ctx.guild.balance}** ${ctx.symbols.avoamusement}
                You have been awarded **${Math.floor(xp)} xp** towards your next rank`)
        }
    })
})
...
cmd(['guild', 'lock'], async (ctx, user, arg1) => {
    ...
    if(ctx.guild.balance < price)
        return ctx.reply(user, `this guild doesn't have **${price}** ${ctx.symbols.avoamusement} required for a lock`, 'red')
    ...
    const question = `Do you want lock this guild to **${col.name}** using **${price}** ${ctx.symbols.avoamusement} ?
        >>> This will add **${guildLock.maintenance}** ${ctx.symbols.avoamusement} to guild maintenance.
