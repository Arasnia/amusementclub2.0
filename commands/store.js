...
cmd('store', 'shop', async (ctx, user, cat) => {
    ...
    const pages = ctx.pgn.getPages(items.map((x, i) => `${i + 1}. [${x.price} ${ctx.symbols.avocado}] \`${x.id}\` **${x.name}** (${x.desc})`), 5)
...
cmd(['store', 'buy'], ['shop', 'buy'], withItem(async (ctx, user, item, args) => {
    if(user.exp < item.price)
        return ctx.reply(user, `you have to have at least \`${item.price}\` ${ctx.symbols.avocado} to buy this item`, 'red')

    return ctx.pgn.addConfirmation(user.discord_id, ctx.msg.channel.id, {
        question: `Do you want to buy **${item.name} ${item.type}** for **${item.price}** ${ctx.symbols.avocado}?`,
        ...
        onConfirm: async (x) => {
            ...
            return ctx.reply(user, `you purchased **${item.name} ${item.type}** for **${item.price}** ${ctx.symbols.avocado}
