...
cmd(['forge'], withMultiQuery(async (ctx, user, cards, parsedargs) => {
    ...
    if(user.exp < cost)
        return ctx.reply(user, `you need at least **${cost}** ${ctx.symbols.avocado} to forge these cards`, 'red')
    ...
    const question = `Do you want to forge ${formatName(card1)}**(x${card1.amount})** and ${formatName(card2)}**(x${card2.amount})** using **${cost}** ${ctx.symbols.avocado}?
