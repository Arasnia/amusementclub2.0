...
module.exports = [
    {
        ...
        resolve: (ctx, user) => {
            user.exp += 200
            return `**200** ${ctx.symbols.avoamusement}`
        }
    }, {
        id: 'auccard',
        ...
        resolve: (ctx, user) => {
            user.exp += 400
            return `**400** ${ctx.symbols.avoamusement}`
        }
    }, {
        id: 'firstdaily',
        name: 'Get the salary',
        ...
        resolve: (ctx, user) => {
            ...
            user.vials += 1000
            user.xp += 100
            return `**10000** ${ctx.symbols.avoamusement} and **1000** ${ctx.symbols.vial}`
        }
    }, {
        id: 'firstforge',
        name: '1+1=1',
        ...
        resolve: (ctx, user) => {
            user.xp += 2
            user.exp += 800
            return `**800** ${ctx.symbols.avoamusement}`
        }
    }, {
        id: 'firstspecial',
        ...
        resolve: (ctx, user) => {
            user.exp += 500
            return `**500** ${ctx.symbols.avoamusement}`
        }
    }, {
        id: '1000stars',
